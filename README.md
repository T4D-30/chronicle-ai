...
│   ├── components/
│   │   ├── adventure/         # AdventureHub, ActionBar, panels
│   │   ├── auth/              # Auth forms
...
**ActionBar owns all player input.** In `AdventureHub`, the `ActionBar` component handles typed text, 8 quick-action shortcuts, and a full JRPG-style combat menu (Attack/Spell/Item/Defend/Move/Flee with weapon/spell/item submenus). Every path calls `submitAction(text)` — the same function typed input uses.
...
**The Adventure Hub is a 3-column pixel-art RPG dashboard (Phase 11), built entirely as an additive UI layer over the existing, unmodified engine/AI Director/session state.** `AdventureLeftNav` (logo, navigation, a real Current Objective card sourced from `campaign.directorConfig.activeThreads`, a compact World Status card, End Session) and `PartyStatusPanel` (portrait, HP bar, a real XP bar computed from `character.experience` against the engine's own `XP_THRESHOLDS`, and a Recent Events list built from the real turn history) are new desktop-only columns; `AdventureScenePanel` adds a location title, a time line, and an honest scene-art placeholder around the **completely unmodified** `StoryPanel`/`ActionBar`. None of this touches game logic — the left nav's navigation buttons call the exact same `onSelectPanel` callback the pre-existing bottom tab nav (still fully intact, still what tablet/mobile actually uses) always used; submitting an action, quick-action buttons, suggested-action chips, dice transparency, and combat entry are all byte-for-byte the same code paths as before. No weather field was invented (none exists in the data model); no party/multiplayer system was invented (this is a single-character "party," honestly presented as such).
