...
## Phase 11 — Adventure Hub UI Redesign ✅
*Goal: redesign the in-game Adventure Hub into a polished 3-column pixel-art RPG dashboard (left nav / center scene / right party status), reusing the existing engine, AI Director, and Adventure Hub state/actions entirely unchanged — UI layer only.*

**Three new, additive components:**
- [x] `AdventureLeftNav` (`src/components/adventure/AdventureLeftNav.tsx`) — desktop-only left navigation column: logo, Home/Characters/Journal/Quests/Codex nav (drives the *same* `activePanel` state the existing bottom tab nav already uses — not a parallel navigation system), Inventory (links to the character sheet's Equipment tab, since no dedicated hub panel exists for it), an honest disabled Settings placeholder (no `/settings` route or mounted settings UI exists anywhere in the app — confirmed by inspection, not assumed), a real Current Objective card (`campaign.directorConfig.activeThreads`, the exact same status/isHidden filter `QuestsPanel` already applies), a compact World Status card (real `worldTime`/current location/discovered count — never weather, which has no field anywhere in the data model), and an End Session button wired to the existing `actions.end()`.
- [x] `AdventureScenePanel` (`AdventureScenePanel.tsx`) — center-column chrome wrapping the **existing, unmodified** `StoryPanel`/suggested-action chips/`ActionBar` as `children`: a location title (resolves `currentLocationId` exactly like `WorldStatusSidebar` already does, falling back to the campaign title — never a fabricated location name), a time line (shown only when `worldState.worldTime` is actually set), and an honest scene-art placeholder (no image-generation system exists or was built; shows the real location `description` text when available, a plain "coming soon" message otherwise — never a fake "generating image…" state implying a capability that doesn't exist).
- [x] `PartyStatusPanel` (`PartyStatusPanel.tsx`) — right-column always-visible summary: portrait, HP bar, and a **real** XP bar (`character.experience`, a genuine DB column, against `XP_THRESHOLDS`/`getXpForNextLevel()`, the exact same engine table `LevelUpModal` already reads from — progress is computed between the current level's floor and the next level's ceiling, not from zero, and a level-20 character shows an honest "Max Level" rather than dividing by a meaningless span), and a Recent Events list built from the real `turns: NarrativeTurn[]` array already flowing through `AdventureState` (no separate "activity feed" concept invented) with a "View Full Journal" button that switches to the existing Journal tab.

**Surgical integration into `AdventureHub.tsx`:** the existing `role="tablist"`/`role="tab"` bottom navigation — the mechanism dozens of pre-existing tests are built against — was not removed, restructured, or duplicated with different semantics. The new left nav calls the exact same `onSelectPanel`/`setActivePanel` callback the bottom tabs already used. `AdventureScenePanel` wraps **only** the `'story'` case inside `ActivePanelContent`'s switch statement; Journal/Quests/Codex/Atlas/Dice/Character/Debug all render exactly as before, with zero scene-art chrome — verified directly by a test asserting `adventure-scene-panel` is absent on every non-story tab. The pre-existing `CharacterSidebar`/`WorldStatusSidebar` columns were not deleted; their breakpoint was bumped from `lg` to `xl` so the redesign's requested 3-column layout (left nav / center / party-status) is the primary `lg+` experience, with the more detailed legacy sidebars appearing as bonus columns on larger `xl+` desktops — an honest, additive breakpoint change, not a removal of tested functionality.

**Three real, informative test failures were found and fixed correctly, not worked around:** adding real new UI that legitimately displays the same real data in more than one place (the campaign title in both the status bar and the new scene panel's location title; a turn's text in both the story scroll and the new Recent Events list; `worldTime` in three real places at once) caused three pre-existing tests using unscoped `getByText` to fail on "multiple elements found." Each was fixed by scoping the query to the specific container the test was actually about (`within(statusBar)`, `within(storyScroll)`, `within(worldStatusSidebar)`) — the correct fix, since the underlying UI behavior was correct and new, not a bug.

**Verification (this branch, independently re-run against the actual `main`-based codebase, not carried over from a combined working copy):** 1995/1995 unit tests passing, 87 files (65 net-new tests across `AdventureLeftNav.test.tsx`, `AdventureScenePanel.test.tsx`, `PartyStatusPanel.test.tsx`, and additions to `AdventureHub.test.tsx`/`AdventurePage.test.tsx`) · 0 TypeScript errors · production build clean (`AdventurePage`'s chunk is 104.51 kB, reflecting the three new components; every other chunk unaffected — `CharacterImportPage` in particular is unchanged, confirming this branch carries no OCR/Vision work) · secret scan clean · integration tests correctly skipped — no persistence, schema, service-layer, or Edge Function code was touched, only client-side components consuming already-loaded `AdventureState`/`AdventureActions`. Two pre-existing, unrelated warnings in `textExtractionParser.test.ts` (confirmed byte-identical to `main`, untouched by this work) do not affect the suite's pass/fail result.

**Explicitly not started this phase, per direct instruction:** Campaign OCR, Living World, interactive maps, fog of war, relationship/reputation systems, combat overhaul, ElevenLabs voice.
