...
| Adventure Hub (3-column redesign) — **superseded** | Historical (Phase 11): the desktop dashboard had a left nav, center scene panel, and party status sidebar, all reusing genuine data and verified not to touch engine/Director/persistence logic. The Unified Adventure Screen recomposed the hub and its cleanup removed those components; Settings, once a disabled placeholder, now works via the pause overlay's Settings tab. | Still true today: there is no image-generation system for scene art, no multiplayer roster ("party" is the one character), and the old sidebar's world-status readout (world time, discovered-location/NPC counts) has not yet been re-surfaced on the unified screen. |
| Playable overworld | Keyboard movement, collision, interactions, dialogue HUD, area transitions, pause-menu reuse, encounter handoff, and combat return all use existing presentation/controller/combat boundaries. Map, exact tile, facing, and the active encounter trigger survive combat handoff locally. | Movement has no touch controls yet. Tile/facing state is presentation-only and is not durable across a full route reload or browser refresh. Fixture encounters are not durable campaign records and can be triggered again after leaving and returning. |
| Unified Adventure Screen | One screen: playable world primary, persistent Story HUD (dialogue + ambient beats, real Director text only), contextual ActionStrip, and every panel as a pause overlay reachable from the bottom nav and Esc. All intents flow through the existing adapter/AdventureActions. | The old world-status readout (world time, discovered-location/NPC counts) has no surface on the unified screen yet. Superseded components (StoryPanel, ActionBar, side navs, AdventureScenePanel, DialogueWindow) were deleted in the unified-screen cleanup. The exploration dice-check display is `CheckResultDock`, docked beside the Story HUD (restored after the cleanup briefly left `lastCheckResult` without a surface). Dialogue speaker portraits are honest procedural placeholders (fixture glyph, deterministic initial tile, or silhouette) — real art lands via the `npc-<id>.png` asset slot; Director-known NPCs are matched to their memory entry by display name, so a renamed NPC gets a fresh tile. Full turn history is only in the Journal overlay; the HUD deliberately shows just the current beat. While a Director response is still streaming, the HUD types the raw token stream — briefly visible as the JSON envelope (`{"narration":"...`) with collapsed whitespace — before the completed beat replaces it with parsed prose; pre-existing behavior from the streaming pipeline, unchanged by Dialogue Cinematics v1, and a candidate for a later streaming-parse pass. |
...
*Last updated: Unified Adventure Screen*
