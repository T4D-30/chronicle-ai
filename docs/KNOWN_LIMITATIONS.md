...
| Unified Adventure Screen | One screen: playable world primary, persistent Story HUD (dialogue + ambient beats, real Director text only), contextual ActionStrip, and every panel as a pause overlay reachable from the bottom nav and Esc. All intents flow through the existing adapter/AdventureActions. | The old world-status readout (world time, discovered-location/NPC counts) has no surface on the unified screen yet. Superseded components (StoryPanel, ActionBar, side navs, AdventureScenePanel, DialogueWindow) were deleted in the unified-screen cleanup. The exploration dice-check display is `CheckResultDock`, docked beside the Story HUD (restored after the cleanup briefly left `lastCheckResult` without a surface). Full turn history is only in the Journal overlay; the HUD deliberately shows just the current beat. |
