...
| Playable overworld | Keyboard movement, collision, interactions, dialogue HUD, area transitions, pause-menu reuse, encounter handoff, and combat return all use existing presentation/controller/combat boundaries | Movement has no touch controls yet. Area and destination spawn survive combat handoff in local presentation state, but exact per-tile position is intentionally not persisted and resets to the area's spawn after combat/remount. Fixture encounters are not durable campaign records and can be triggered again after leaving and returning. |
