...
| Unified Adventure Screen | One screen: playable world primary, persistent Story HUD (dialogue + ambient beats, real Director text only), contextual ActionStrip, and every panel as a pause overlay reachable from the bottom nav and Esc. All intents flow through the existing adapter/AdventureActions. | The old world-status readout (world time, discovered-location/NPC counts) has no surface on the unified screen yet. Superseded components (StoryPanel, ActionBar, side navs, AdventureScenePanel, DialogueWindow) were deleted in the unified-screen cleanup. The exploration dice-check popup lost its surface with StoryPanel — `lastCheckResult` is still produced by the session hook but nothing renders it yet (re-surfacing it on the unified screen is follow-up work). Full turn history is only in the Journal overlay; the HUD deliberately shows just the current beat. |
