...
- Verification results: `npx tsc --noEmit`, `npx vitest run` (2,248 tests), and `npm run build:check` pass per commit; authenticated browser walkthroughs at 1280×800 and 390×844 including NPC dialogue, ambient beats, all overlays, focus trap, reduced motion, and encounter → combat → exact-position return, with zero console errors.
